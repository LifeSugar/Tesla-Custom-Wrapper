Shader "Hidden/Tesla_Bake_ObjectNormal"
{
    Properties
    {
        // 不需要任何贴图输入，我们只读取模型的法线数据
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            struct appdata
            {
                ...
                float3 normal : NORMAL;    // 模型的法线（已经是物体空间）
            ...
            struct v2f
            {
                ...
                float3 objNormal : TEXCOORD0; // 要存储的数据（物体空间法线）
            };
            ...
            v2f vert (appdata v)
            {
                ...
                // ----------------------------------------------------
                // 【传递数据】
                // 我们要把什么数据存进贴图里？这里存的是 Object Space Normal
                // 注意：Unity 的 v.normal 本身就是物体空间法线，无需转换
                // ----------------------------------------------------
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                // 把法线 (x, y, z) 直接当作颜色 (r, g, b) 输出
                // 注意：法线范围是 -1~1，而颜色范围是 0~1
                // 对于物体空间法线，我们直接存储原始值（支持负值，使用浮点格式）
